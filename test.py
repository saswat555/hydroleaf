...
def test_auth_and_users():
    ...
    rr(session.put(f"{API}/users/me", headers=user_headers,
       ...
       expect=200)
    rr(session.post(f"{API}/auth/logout", headers=user_headers), expect=200)
    ...
    rr(session.delete(f"{ADMIN}/users/{user_id}", headers=admin_headers), expect=200)
...
def test_farm_crud(user_headers):
    log.info("=== FARM CRUD ===")
    data = {"name":"Greenhouse","location":"Sector 9"}
    create = rr(session.post(f"{API}/farms", headers=user_headers, json=data), expect=200).json()
    fid = create["id"]
    rr(session.get(f"{API}/farms", headers=user_headers), expect=200)
    rr(session.get(f"{API}/farms/{fid}", headers=user_headers), expect=200)
    rr(session.put(f"{API}/farms/{fid}", headers=user_headers,
                   json={"name":"GH Updated","location":"Sector 10"}), expect=200)
...
def test_device_registration_and_discovery(user_headers):
    ...
    return did, sid, esid, vid


def test_device_comm_endpoints(user_headers, admin_headers, did, vid):
    log.info("=== DEVICE COMM ===")
    rr(session.get(f"{API}/device_comm/update", params={"device_id":"AA:BB:CC:DD"}), expect=200)
    rr(session.get(f"{API}/device_comm/update/pull", params={"device_id":"AA:BB:CC:DD"}), allow=[200,404])
    rr(session.get(f"{API}/device_comm/pending_tasks", params={"device_id":"AA:BB:CC:DD"}), expect=200)
    rr(session.post(f"{API}/device_comm/tasks", params={"device_id":"AA:BB:CC:DD"}, json={"pump":1,"amount":50}), expect=200)
    rr(session.post(f"{API}/device_comm/heartbeat", headers=admin_headers,
                    json={"device_id":"AA:BB:CC:DD","type":"dosing_unit","version":"3.0.0"}), expect=200)
    rr(session.post(f"{API}/device_comm/valve_event", json={"device_id":"VV:AA:LL:11","valve_id":2,"state":"on"}), expect=200)
    rr(session.get(f"{API}/device_comm/valve/VV:AA:LL:11/state"), expect=200)
    rr(session.post(f"{API}/device_comm/valve/VV:AA:LL:11/toggle", json={"valve_id":vid}), expect=200)
...
def test_config_and_dosing_profile(user_headers, did):
    ...
    prof = rr(session.post(f"{API}/config/dosing-profile", headers=user_headers,
                           json={
    ...
    rr(session.put(f"{API}/config/dosing-profiles/{pid}", headers=user_headers,
                   json={"dosing_schedule":{"09:00":12.0}}), expect=200)
...
def test_dosing_endpoints(user_headers, did):
    ...
    rr(session.get(f"{API}/dosing/profiles", headers=user_headers), expect=200)
...
def test_plants(user_headers):
    ...
    created = rr(session.post(f"{API}/plants", headers=user_headers,
                              json={
    ...
    rr(session.put(f"{API}/plants/{pid}", headers=user_headers,
                   json={"growth_stage":"vegetative"}), expect=200)
    rr(session.delete(f"{API}/plants/{pid}", headers=user_headers), expect=200)
    rr(session.post(f"{API}/plants/execute-dosing/{pid}", headers=user_headers), allow=[400,404])


def test_subscriptions(user_headers):
    log.info("=== SUBSCRIPTIONS ===")
    rr(session.get(f"{API}/subscriptions/plans", headers=user_headers), expect=200)
    plans = rr(session.get(f"{API}/subscriptions", headers=user_headers), expect=200).json()
    if plans:
        sid = plans[0]["id"]
        rr(session.get(f"{API}/subscriptions/{sid}", headers=user_headers), allow=[200,404])


def test_payments(user_headers, admin_headers):
    log.info("=== PAYMENTS ===")
    order = rr(session.post(f"{API}/payments/create", headers=user_headers,
                            json={"device_id":1,"plan_id":1}), expect=201).json()
    oid = order["id"]
    rr(session.get(f"{API}/payments/{oid}", headers=user_headers), allow=[200,404])
    rr(session.post(f"{API}/payments/confirm/{oid}", headers=user_headers,
                    json={"upi_transaction_id":"TXN123"}), expect=200)
    ...
    rr(session.post(f"{ADMIN}/payments/cancel/{oid}", headers=admin_headers), allow=[200,404])


def test_cloud(admin_headers, user_headers):
    log.info("=== CLOUD AUTH ===")
    new = rr(session.post(f"{API}/cloud/admin/generate_cloud_key", headers=admin_headers), expect=200).json()
    key = new["cloud_key"]
    rr(session.post(f"{API}/cloud/verify_key", json={"device_id":"X","cloud_key":key}), expect=200)
    rr(session.post(f"{API}/cloud/authenticate", json={"device_id":"X","cloud_key":key}), expect=200)
    rr(session.get(f"{API}/cloud/keys", headers=admin_headers), allow=[200,404])
    rr(session.post(f"{API}/cloud/dosing_cancel", json={"device_id":"X","event":"foo"}), expect=400)
    rr(session.post(f"{API}/cloud/dosing_cancel", json={"device_id":"X","event":"dosing_cancelled"}), expect=200)


def test_supply_chain(user_headers):
    log.info("=== SUPPLY CHAIN ===")
    payload = {"origin":"Rewa","destination":"Delhi","produce_type":"Mango","weight_kg":100,"transport_mode":"railway"}
    sc = rr(session.post(f"{API}/supply_chain", headers=user_headers, json=payload), expect=200).json()
    scid = sc.get("id")
    if scid:
        rr(session.get(f"{API}/supply_chain/{scid}", headers=user_headers), allow=[200,404])


def test_admin_devices():
    log.info("=== ADMIN DEVICES ===")
...
def test_cameras():
    log.info("=== CAMERAS ===")
    cam_id = "test_cam"
    rr(session.post(f"{API}/cameras/upload/{cam_id}/day", data=IMG_BYTES, headers={"Content-Type":"image/jpeg"}), expect=200)
    rr(session.post(f"{API}/cameras/upload/{cam_id}/night", data=IMG_BYTES, headers={"Content-Type":"image/jpeg"}), expect=200)
...
def test_activation_and_subscriptions(admin_headers, user_headers, device_ids):
    log.info("=== ACTIVATION & SUBSCRIPTIONS ===")
    for dev in device_ids:
        ak_resp = session.post(f"{ADMIN}/devices/{dev}/activation_key", headers=admin_headers)
        ak = rr(ak_resp, expect=201).json().get("activation_key")
        sub_resp = session.post(f"{API}/subscriptions/redeem", headers=user_headers, json={"activation_key": ak, "device_id": dev})
        rr(sub_resp, expect=200)
...
if __name__ == "__main__":
    ...
    did, sid, esid, vid = test_device_registration_and_discovery(user_headers)
    test_device_comm_endpoints(user_headers, admin_headers, did, vid)
    ...
    test_admin_devices()
    test_cameras()
    test_activation_and_subscriptions(admin_headers, user_headers, [did, sid, esid, vid])
