...
from app.core.config import (
    DATA_ROOT, RAW_DIR, CLIPS_DIR,
    RETENTION_DAYS, HLS_TARGET_DURATION,
    HLS_PLAYLIST_LENGTH, OFFLINE_TIMEOUT, FPS,
)
...
async def _segment_hls(raw_path: Path):
    """Call system ffmpeg to produce HLS."""
...
async def _generate_cv_version(raw_path: Path):
    """Re-encode raw_path → rawname_cv.mp4 with YOLO overlay."""
    stem   = raw_path.stem
    cv_path = raw_path.parent / f"{stem}_cv.mp4"
    cap    = cv2.VideoCapture(str(raw_path))
    if not cap.isOpened():
        logger.error(f"Cannot open {raw_path} for CV processing")
        return
...
def _detect_sync(img):
    """Blocking YOLO inference + draw boxes."""
...
async def encode_and_cleanup(cam_id: str):
    """
    1. Batch all new JPEGs
    2. Write them to the raw MP4
    3. Rollover per duration or auto-delay
    4. Cleanup JPEGs & prune old MP4s
    """
    ...
    async with lock:
        ...
        frames = sorted(
            [f for f in raw_dir.glob("*.jpg") if f.stat().st_size > 1000],
